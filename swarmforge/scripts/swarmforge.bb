...
(defn launch-command [ctx index row]
  (let [role (:role row)
    ...
    (cond-> (str base
                (case agent
                  "claude" (str "claude --append-system-prompt-file " (sq (str prompt-file)) " --permission-mode default -n " (sq (str "SwarmForge " display)) " " (extra-args-prefix row) "\"$(cat " (sq (str prompt-file)) ")\"")
